...
class TestServer(unittest.TestCase):
    ...
    def test_read_request(self, mock_socket):
        ...
        def fake_recv():
            return_values = ["url_1", "url_2", "url_3", "url_4", "url_5", None]
        ...
        self.client_socket = self.server.accept_client_conn(self.server_socket)
        ...
        for fake_request, expected_request in zip(
            ...
            ["url_1", "url_2", "url_3", "url_4", "url_5", None],
        ):
    ...
    def test_read_request_exception(self, mock_socket):
        ...
        self.assertRaises(ConnectionResetError, mock_socket.recv)
    ...
    @patch("server.my_server.socket.socket")
    def test_handle_valid_request(self, mock_socket):
        ...
        self.client_fake_request = self.server.read_request(self.client_socket)
        ...
        self.server.handle_request(client_request=self.client_fake_request, client_sock=self.client_socket)
    ...
    @patch("server.my_server.socket.socket")
    def test_handle_invalid_request(self, mock_socket):
        """Mock the requests.get"""
    ...
    def test_parse_response(self):
        ...
        self.assertEqual(json_tools.diff(self.json_response, self.json_expected), [])
    ...
    def test_num_of_processed_urls(self, mock_socket):
        ...
        def fake_recv():
            return_values = [str(i).encode("utf-8") for i in range(0, self.num_of_fake_urls)]
            for value in return_values:
                yield value
        ...
        with ThreadPool(self.server._k_top) as pool:
            for client_request in self.server.read_request(self.client_socket):
                pool.add_task(
                    self.server.handle_request, client_request, self.client_socket
                )
    ...
    #             self.index = 0
    #             self.num_of_fake_urls = num_of_urls
    #             self.values = [str(i) for i in range(0, self.num_of_fake_urls)]
    #
    #         def __next__(self):
    ...
    #
    #     # mock_socket.recv.return_value = next(iterable_seq)
    #
    #     mock_requests_get = mock_requests_get_patcher.start()
    #     mock_requests_get.return_value.text.return_value = "fake response"
    #
